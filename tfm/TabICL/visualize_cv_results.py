#!/usr/bin/env python
"""
TabICL CV Results Visualization (Modular Version)
=================================================
...
# CONFIGURATION
# ==========================================
BASE_RESULTS_DIR = Path('/vol/miltank/users/sajb/Project/NeuralFineGray/tabICL/results')
...
def plot1_tabicl_impact(data, plots_dir, dataset_config):
    """
    ...
    bars3 = ax.bar(x + width, dr_vals, width, label=f'TabICL + Raw ({512+n_features}D)', 
                   color=MODE_COLORS['Deep+Raw'], edgecolor='white', linewidth=1.5)
    
    # Formatting
    ax.set_title(f'TabICL Impact on Survival Models - {dataset_name}\n(C-Index at Median Survival Time, 5-Fold CV)', 
    ...
    # Find model with best improvement from TabICL
    improvements = []
    ...
    if improvements:
        ...
        insight_text = f"Best Model: {best_model} (C-Index: {best_raw:.3f})\nBest TabICL Improvement: {best_improve[0]} ({best_improve[1]:+.1f}%)"
    else:
        insight_text = f"Best Model: {best_model} (C-Index: {best_raw:.3f})"
...
def plot2_temporal_performance(data, plots_dir, dataset_config):
    ...
    ax2.set_title(f'TabICL + Raw Features ({512+n_features}D)', fontsize=13, fontweight='bold')
...
def plot3_discrimination_vs_calibration(data, plots_dir, dataset_config):
    ...
    n_features = dataset_config['n_features']
    mode_labels = {'Raw': f'Raw ({n_features}D)', 'Deep': 'TabICL (512D)', 'Deep+Raw': f'TabICL+Raw ({512+n_features}D)'}
...
def print_summary_table(data, dataset_config):
    ...
    # Find model with best TabICL improvement
    best_improvement = 0
    best_improve_model = None
    for model in MODELS_TABICL:
        raw_c = data['Raw'].get(model, {}).get('c_index_q50', 0)
        dr_c = data['Deep+Raw'].get(model, {}).get('c_index_q50', 0)
    ...
    if best_improve_model:
        ...
        print(f"2. {best_improve_model} benefits most from TabICL: {raw_c:.4f} → {dr_c:.4f} (+{best_improvement:.1f}%)")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Visualize TabICL CV Results',
...
def main():
    ...
    print("="*60)
    print(f"TabICL CV Results Visualization - {dataset_config['name']}")
    ...
    if data is None:
        ...
        print(f"Run: python run_cv_analysis.py --dataset {args.dataset}")
        return
    ...
    plot1_tabicl_impact(data, plots_dir, dataset_config)
