#!/usr/bin/env python
"""
Cross-Validation Analysis Pipeline for TabICL Embeddings

This script orchestrates:
1. 5-Fold Cross-Validation
2. Hyperparameter Tuning (Random Search) within each fold
3. TabICL embeddings generated dynamically inside folds (transductive setup)
4. Evaluation at time quantiles (q0.25, q0.50, q0.75)

Usage:
    python run_cv_analysis.py                    # Default: METABRIC
    python run_cv_analysis.py --dataset PBC      # Run on PBC
    python run_cv_analysis.py --dataset SUPPORT  # Run on SUPPORT
"""
...
from experiments.experiment import (
    ...
    DeepSurvExperiment, TabICLExperiment
)
...
RANDOM_SEED = 42
N_FOLDS = 5
N_ITER = 20  # Random search iterations per fold
...
# Models that should use PCA compression for TabICL embeddings
...
def run_experiment(model_name, mode, dataset='METABRIC', results_dir=None, verbose=True):
    ...
    try:
        ...
        # Use special grid for DeepSurv with TabICL embeddings
        ...
        if mode == 'raw':
            # Standard experiment without TabICL
            ...
            # TabICL-enhanced experiment
            exp = TabICLExperiment(
                ...
                # Model-specific TabICL settings
...
def main(dataset='METABRIC'):
    ...
    print("="*60)
    print(f"CROSS-VALIDATION ANALYSIS: TabICL on {dataset}")
    print("="*60)
